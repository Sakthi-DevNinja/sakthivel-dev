...
export const ProjectCard: React.FC<{ project: SystemWork }> = ({ project }) => {
  return (
    <div className="glass rounded-4xl p-8 md:p-12 glass-hover transition-all duration-500 flex flex-col h-full relative group">
      ...
      <div className="space-y-8 grow">
        {project.objective && (
          <div>
            <span className="mono text-[11px] uppercase tracking-[0.3em] text-blue-500/70 block mb-2">Objective</span>
        ...
        {project.overview && (
          <div>
            <span className="mono text-[11px] uppercase tracking-[0.3em] text-blue-500/70 block mb-2">Overview</span>
        ...
        {project.role && (
          <div>
            <span className="mono text-[11px] uppercase tracking-[0.3em] text-blue-500/70 block mb-2">Primary Role</span>
        ...
        {project.contribution && (
          <div>
            <span className="mono text-[11px] uppercase tracking-[0.3em] text-blue-500/70 block mb-3">Key Contribution</span>
        ...
        {project.features && (
          <div>
            <span className="mono text-[11px] uppercase tracking-[0.3em] text-blue-500/70 block mb-3">System Capabilities</span>
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
              {project.features.map((f, i) => (
                <li key={i} className="text-xs text-neutral-400 flex items-start gap-3">
                  <span className="w-1 h-px bg-blue-500 mt-2 shrink-0"></span>
                  {f}
                </li>
              ))}
            </ul>
        ...
        {project.impact && (
          <div className="bg-blue-500/5 p-4 rounded-xl border border-blue-500/10">
            <span className="mono text-[11px] uppercase tracking-[0.3em] text-blue-400 block mb-1">Impact</span>
      ...
      <footer className="mt-12 pt-8 border-t border-white/5 flex flex-wrap gap-2">
        {project.technologies?.map(tech => (
          <span key={tech} className="px-3 py-1 bg-blue-600/20 rounded-md text-[10px] mono text-white hover:text-blue-300 border border-white/5">{tech}</span>
        ))}
        {project.note && (
